 ...
 * with the object for attention — which is the brief for frame 01.
 */
export function ScrollIndicator() {
  const apply = useCallback((element: HTMLDivElement, progress: number) => {
    ...
    if (thumb) {
      thumb.style.transform = `translate3d(0, ${clamp(progress) * 62}px, 0)`;
    }
    // Steps aside once the Home has assembled — the page takes over from there.
    element.style.opacity = String(window4(progress, -0.02, -0.005, 0.95, 1.0) * 0.9);
  }, []);
